...
from hack_utils import RandomAugmentation, RandomAffineAugmenter, MirrorAugmenter, BrightnessContrastAugmenter, BlurAugmenter
...
def train(model, loader, loss_fn, optimizer, device):
    ...
    for batch in tqdm.tqdm(loader, total=len(loader), desc="training..."):
        ...
        sizes = batch["size"].squeeze() # B

        min_size = torch.min(sizes)
        max_size = torch.max(sizes)
        weightes = 1.0 - 0.9 * (sizes - min_size) / (max_size - min_size)
        weightes.requires_grad_(False)

        pred_landmarks = model(images).cpu()  # B x (2 * NUM_PTS)

        # Изменение 1: взвешивание лоссов по размеру входного изображения
        # Была гипотеза (визуально), что кривая разметка чаще встречается на больших изображениях
        losses = loss_fn(pred_landmarks, landmarks, reduction="none")
        losses = torch.mean(losses, 1)
        loss = torch.dot(losses, weightes)
...
def main(args):
    ...
    train_transforms = transforms.Compose([
        ...
        # Изменение 2: аугментация
        RandomAugmentation(
            RandomAffineAugmenter(),
#             MirrorAugmenter(),
